# Ken Imoto — Publications

## Research Papers (Zenodo)

### Excess Vocabulary in Japanese AI-Generated Text: A Cross-Model Quantitative Analysis
- DOI: 10.5281/zenodo.19233934
- Date: March 2026
- Dataset: 350 AI samples (7 models) vs. 977 human articles
- Key finding: 651 statistically significant excess words; cross-domain classifier AUC=0.946 but fails across genres
- Code & Data: https://github.com/kenimo49/excess-vocabulary-ja

### AI Text Slop: A Quantitative Study of Stylistic Convergence Across Six Language Models in Japanese Technical Writing
- DOI: 10.5281/zenodo.19173035
- Date: March 2026
- Dataset: 180 samples (6 models x 10 topics x 3 trials)
- Key finding: RLHF models score higher on slop metrics (Cohen's d = 1.01); Swallow Paradox
- Code & Data: https://github.com/kenimo49/ai-text-slop

### AI Blue: Systematic Color Recognition Bias in Vision-Language Models
- DOI: 10.5281/zenodo.19159702
- Date: March 2026
- Dataset: 4 VLMs x 40 colors x 480 observations
- Key finding: 95% of AI-generated UI colors land on blue-purple
- Code & Data: https://github.com/kenimo49/ai-blue-color-bias

## Books — AI Development Series

### AIコードレビューを仕組み化する技術 — レビュー時間60%削減の3層モデル
- Topics: AI Code Review, 3-Layer Model, Review Automation, Code Quality
- Kindle JP: https://amzn.asia/d/0hgvOnOi
- Zenn: https://zenn.dev/kenimo49/books/harness-code-review (¥1,000)

### Practical Claude Code — Context Engineering for Modern Development
- Topics: Claude Code, Context Engineering, CLAUDE.md, AI-native development
- Kindle JP: https://amzn.asia/d/03Qnb8QT
- Kindle EN: https://amzn.asia/d/0hU3BP1i
- Zenn: https://zenn.dev/kenimo49/books/claude-code-mastery (¥1,000)

### Context Engineering in Practice — Turning LLMs from Liars into Experts
- Topics: Context Engineering, RAG, MCP, LLM hallucination, benchmark experiments
- Achievement: Up to 4.6x quality improvement measured with original benchmarks
- 96 production-ready Python code files verified on GitHub
- Kindle JP: https://amzn.asia/d/04rulUoN
- Zenn: https://zenn.dev/kenimo49/books/context-engineering (¥1,500)
- Code: https://github.com/kenimo49/context-engineering-benchmark

### Claude Code QuickStart
- Topics: Claude Code, Getting Started, AI Development
- Zenn: https://zenn.dev/kenimo49/books/claude-code-quickstart (Free)

## Books — LLMO / AI Search Optimization Series

### LLMO Practical Guide — Why ChatGPT Ignores Your Website
- Topics: LLMO, LLM Optimization, SEO, Structured Data, JSON-LD, llms.txt, AI Search
- Kindle JP: https://amzn.asia/d/02zAl1VB
- Kindle EN: https://amzn.asia/d/04rulUoN
- Zenn: https://zenn.dev/kenimo49/books/llmo-ai-search-optimization (¥1,000)

### LLMO Quickstart — AI Search Optimization for Engineers
- Topics: LLMO, AI Search, Quick Guide
- Kindle JP: https://amzn.asia/d/04skv9ST (¥300)
- Kindle EN: https://amzn.asia/d/04XYlHKe

### LLMO Website Builder
- Topics: LLMO-optimized website development from scratch
- Zenn: https://zenn.dev/kenimo49/books/llmo-website-builder (¥500)

## Books — Security & Quality Series

### MCP Security in Practice — What OWASP Won't Tell You About AI Tool Integrations
- Topics: MCP, Model Context Protocol, Security, OWASP MCP Top 10, Token Cost, 7 services tested
- Kindle JP: https://amzn.asia/d/0cQpLy6g (¥500)
- Kindle EN: https://amzn.asia/d/03ceMosL ($4.99)
- Zenn: https://zenn.dev/kenimo49/books/mcp-security-practice (Free)

### AI Slop Escape Guide
- Topics: AI Slop, UI color bias, AI Blue, design differentiation
- Zenn: https://zenn.dev/kenimo49/books/ai-slop-escape-guide (¥500)

## Books — Knowledge & Data Series

### ナレッジグラフ活用大全 — 構造化すれば、AIは賢くなる
- Topics: Knowledge Graph, GraphRAG, Neo4j, RDF, Property Graph, Tree-sitter, MCP, Emotion AI
- Zenn: https://zenn.dev/kenimo49/books/knowledge-graph-practical-guide (¥1,500)

## Technical Writing

- **Qiita**: 67,000+ PV, 25+ articles — https://qiita.com/kenimo49
- **Zenn**: 13+ articles + 8 Books — https://zenn.dev/kenimo49
- **DEV.to**: English technical articles — https://dev.to/kenimo49
- **LinkedIn**: English AI/LLM articles — https://linkedin.com/in/kenimo49
- **Amazon Author**: https://www.amazon.co.jp/stores/author/B0GQNPRCGF
