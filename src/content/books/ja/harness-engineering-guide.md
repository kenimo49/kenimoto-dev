...
ハーネスエンジニアリングという言葉が独り歩きしています。OpenAIは「Codexのスケーラビリティ」を語り、Anthropicは「長時間実行エージェント」を語り、LangChainは「Agent = Model + Harness」と整理する。Martin Fowlerは「コードベースに既に存在する暗黙のハーネス」を指摘する。

それぞれが正しい。しかし、これらをひとつの体系として整理した本は、これまで存在しませんでした。

本書は **ハーネスとは何か、どう設計するか、どう運用するか** を、5社の解釈を統合して6つの構成要素に落とし込み、AGENTS.md / CLAUDE.md / hooks の実装パターンまで一気通貫で扱います。

> 「2024年のプロンプト、2025年のコンテキスト、2026年のハーネス。」
